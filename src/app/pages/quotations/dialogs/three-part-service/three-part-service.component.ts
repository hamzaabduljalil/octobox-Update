...
export class ThreePartServiceComponent {
   dynamicDialogRef = inject(DynamicDialogRef);
    ...
    getData(){
      ...
      const cities = this.dynamicDialogConfig.data?.obj.city.map(city => city.id);
        ...
        next: (value) => {
          ...
          if (this.dynamicDialogConfig.data?.obj.area) {
            this.areas.forEach((parentArea, cityIndex) => {
              const selectedAreasForCity = parentArea.areas.filter(area =>
  ...
  save() {
    ...
    console.log(this.dynamicDialogConfig.data.obj)
    const headerText = this.translate.instant(this.dynamicDialogConfig.data?.obj ? "Edit" : "Add")
